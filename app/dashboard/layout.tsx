...
function aktifSekmeBul(pathname: string): SekmeKey {
  if (pathname === '/dashboard') return 'ozet'
  const sekme = SEKMELER.find((s) => s.key !== 'ozet' && pathname.startsWith(s.href))
  return sekme?.key || 'ozet'
}

// Bu layout, /dashboard altındaki HER sayfayı otomatik sarar — sayfalar arasında
// gezinirken kenar çubuğu hiç yeniden yüklenmez, sadece sağdaki içerik değişir.
    ...
    <div className="min-h-screen bg-paper md:flex">
      <aside className="hidden md:flex md:flex-col md:w-60 md:shrink-0 bg-navy min-h-screen sticky top-0 self-start">
        ...
        <div className="px-3 pb-4 flex flex-col gap-1 border-t border-paper/10 pt-3">
          <ProfilModal
            tetikleyici={
              <button className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-paper/60 hover:bg-paper/5 hover:text-paper transition-colors text-left">
                <span>👤</span>Profil
              </button>
            }
          />
      ...
      <div className="flex-1 md:min-w-0">
        <header className="md:hidden bg-navy px-6 py-4 flex items-center justify-between sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3">
            <ProfilModal
              tetikleyici={
                <button className="text-paper/80 hover:text-paper p-1.5" aria-label="Profilim">
