...
export default function BirikimEkleModal() {
  ...
  function sifirlaVeKapat() {
    setAcik(false)
    setGoalName(''); setTargetAmount(''); setTargetDate(''); setMessage('')
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!tutar || tutar <= 0) { setMessage('Geçerli bir hedef tutar gir.'); return }
    ...
    const { error } = await supabase.from('savings_goals').insert({
      user_id: user.id, goal_name: goalName, target_amount: tutar, current_amount: 0, target_date: targetDate || null,
    })
  ...
  return (
    <>
      ...
      <Modal acik={acik} baslik="Yeni Hedef Ekle" onKapat={sifirlaVeKapat}>
