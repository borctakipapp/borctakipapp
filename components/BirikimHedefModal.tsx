...
// Kullanım: hedef kartına bu bileşeni koy, goalId prop'u geçir, üstüne bir tetikleyici (children) sar.
export default function BirikimHedefModal({ goalId, tetikleyici }: { goalId: string; tetikleyici: React.ReactNode }) {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    const { data: goal, error } = await supabase.from('savings_goals').select('*').eq('id', goalId).single()
    if (goal) {
      setGoalName(goal.goal_name); setTargetAmount(String(goal.target_amount))
      setCurrentAmount(String(goal.current_amount)); setTargetDate(goal.target_date || '')
    }
    if (error) setMessage('Hedef bulunamadı.')

    const { data: entryList } = await supabase.from('savings_entries').select('*').eq('goal_id', goalId).order('created_at', { ascending: false })
  ...
  async function handleAddEntry(e: React.FormEvent) {
    ...
    if (entryType === 'withdraw' && gelirOlarakEkle) {
      ...
      const { error } = await supabase.rpc('gelir_gider_ekle_ve_aktar', {
        p_user_id: user.id, p_type: 'income', p_category: 'Birikimden Çekim', p_amount: tutar,
  ...
  const tamamlandi = targetAmount ? parseFloat(currentAmount) >= parseFloat(targetAmount) : false
      ...
      <Modal acik={acik} baslik={goalName || 'Hedef'} onKapat={() => setAcik(false)}>
        {loading ? (
          ...
          <>
            ...
            <div className="h-2.5 bg-paper border border-border rounded-full overflow-hidden mb-6">
            ...
            <div className="bg-sage-soft rounded-lg p-4 mb-6">
              ...
              <p className="text-[11px] text-muted mb-3">
                Bu, Gelir-Gider'ini etkilemez — sadece bu hedefin toplam tutarını günceller.
              </p>
              <form onSubmit={handleAddEntry} className="flex flex-col gap-2">
                <div className="flex gap-2">
                  <button type="button" onClick={() => setEntryType('add')}
                    className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${entryType === 'add' ? 'bg-sage text-white' : 'bg-white border border-border text-muted'}`}>
                    Para Ekle
                  </button>
                  <button type="button" onClick={() => setEntryType('withdraw')}
                    className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${entryType === 'withdraw' ? 'bg-brick text-white' : 'bg-white border border-border text-muted'}`}>
                    Para Çek
                  </button>
                </div>
                <input type="number" step="0.01" placeholder="Tutar (₺)" value={entryAmount} onChange={(e) => setEntryAmount(e.target.value)}
                ...
                {entryType === 'withdraw' && (
                  <label className="flex items-start gap-2 text-xs text-muted bg-white border border-border rounded-lg p-2.5">
                    ...
                    <span>Bu tutarı gelir olarak da ekle (Gelir-Gider'e yansısın)</span>
                  </label>
                )}
