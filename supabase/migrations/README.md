...
- `20260102000000_fix_admin_izinleri_missing_table.sql` — eksik tabloyu oluşturan bug fix (1/2)
- `20260103000000_fix_admin_izinleri_add_veren_id.sql` — eksik `veren_id` kolonu + `izin` CHECK kısıtı (2/2, canlı testte bulundu)
...
- ~~`admin_izinleri.izin` kolonunun olası değerleri için CHECK kısıtı~~ — **çözüldü** (`20260103000000`), `lib/admin-auth.ts`'deki `TUM_YETKILER` sabitinden alındı.
- ~~`admin_izinleri`'nin yazma tarafında RLS policy'si yok~~ — **doğrulandı, sorun değil**: `lib/admin-yetki-actions.ts` gerçekten `createAdminClient()` (service-role) kullanıyor, RLS'i bilinçli olarak bypass ediyor. Ekstra INSERT/DELETE policy gerekmiyor.
